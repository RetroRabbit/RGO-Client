  ...
  onFileChange(event: any): void {
    if (event.target.files && event.target.files.length) {
      ...
      this.fileToArrayBuffer(file, (arrayBuffer) => {
        ...
        const compressedData = this.compressAndSerializeFile(arrayBuffer);
        ...
        const deserializedArrayBuffer = this.deserializeAndDecompressFile(compressedData);

        // Reconstruct the file and trigger the download
        // this.downloadArrayBufferAsFile(deserializedArrayBuffer, file.name, file.type);
      });
